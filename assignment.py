...
for record in number:                           #Cycle through each record in the list
    ...
    message.say("Hello " + name[count])         #Say hello and concact the name of the current record
    print(str(message))                         #Print it for callers reference
    time.sleep(3)                               #Small time delay
    call.hangup()                               #Hang the call up (change sleep to change the time it takes to hang up)
